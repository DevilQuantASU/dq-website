import React from 'react';

const Footer = () => {
    return (
        <footer className="bg-neutral-900 border-t border-neutral-800">
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row md:justify-between gap-8 md:gap-16">
                    <div className="space-y-8 max-w-sm">
                        <div>
                            <span className="font-bold text-xl tracking-tight text-white block">
                                DevilQuant
                            </span>
                            <p className="text-neutral-400 text-sm mt-4">
                                Advanced quantitative analysis for modern markets.
                            </p>
                        </div>
                        <div className="flex space-x-6">
                            <a href="https://github.com/DevilQuant" target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-neutral-300">
                                <span className="sr-only">GitHub</span>
                                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                                </svg>
                            </a>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-8 md:gap-16">
                        <div>
                            <h3 className="text-sm font-semibold text-neutral-300 tracking-wider uppercase">
                                Connect
                            </h3>
                            <ul className="mt-4 space-y-4">
                                <li>
                                    <a href="/linkedin" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        LinkedIn
                                    </a>
                                </li>
                                <li>
                                    <a href="/sundevilcentral" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        Sun Devil Central
                                    </a>
                                </li>
                                <li>
                                    <a href="/discord" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        Discord
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div>
                            <h3 className="text-sm font-semibold text-neutral-300 tracking-wider uppercase">
                                Resources
                            </h3>
                            <ul className="mt-4 space-y-4">
                                <li>
                                    <a href="/resources?guide=getting-started-with-quant" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        Getting Started
                                    </a>
                                </li>
                                <li>
                                    <a href="/resources?guide=landing-an-internship" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        Internships
                                    </a>
                                </li>
                                <li>
                                    <a href="/resources?guide=campus-resources" className="text-base text-neutral-400 hover:text-white transition-colors">
                                        Campus Resources
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div className="mt-12 border-t border-neutral-800 pt-8">
                    <p className="text-base text-neutral-400 xl:text-center">
                        &copy; 2026 DevilQuant. All rights reserved.
                    </p>
                </div>
            </div>
        </footer>
    );
};

export default Footer;
